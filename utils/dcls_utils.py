...
def get_uperleft_denominator(img, kernel, grad_kernel):
    ...
    img = torch.fft.fftn(img, dim=(-3, -2, -1))
    numerator = torch.stack((img.real, img.imag), -1)
...
def deconv(inv_ker_f, fft_input_blur):
    ...
    deblur = torch.fft.ifftn(torch.complex(deblur_f[..., 0], deblur_f[..., 1]), dim=(-3, -2, -1))
...
def convert_psf2otf(ker, size):
    psf = torch.zeros(size, device=ker.device, dtype=ker.dtype)
    # circularly shift
    ...
    img = torch.fft.fftn(psf, dim=(-3, -2, -1))
    otf = torch.stack((img.real, img.imag), -1)
...
def convert_otf2psf(otf, size):
    ker = torch.zeros(size, device=otf.device, dtype=otf.dtype)
    # psf = torch.irfft(otf, 3, onesided=False)

    psf = torch.fft.ifftn(torch.complex(otf[..., 0], otf[..., 1]), dim=(-3, -2, -1))
